...
\begin{exercise}
	...
	\begin{enumerate}
		...
		\end{equation*}
		\item Soit $M\in\mathcal{S}_n(\R)$ telle que 
		\begin{enumerate}[label=\roman*)]
			\item Sur chaque ligne, $g$ coefficients valent 1 et les autres 0
			\item $\forall i\in\left\llbracket1,\right\rrbracket$, $m_{ii}=0$
			\item $\forall i\neq j$, si $m_{ij}=1$ alors $\exists! k\in\left\llbracket 1,n\right\rrbracket$, $m_{ki}=m_{kj}=1$. Si $m_{ij}=0$ alors il n'y a pas de tel indice.
		\end{enumerate}
		Évaluer $MJ,JM$ et $M^{2}$ en fonction de $M,J,\mathrm{I}_n$. Montrer que $d=0$ ou $d=2$ et $\im J\subset\ker(M-d\mathrm{I}_n)$.
	\end{enumerate}
\end{exercise}

\begin{exercise}
	Soit $(A,B,C)\in\M_n(\C)^{3}$ telles que $CA=BC$ et $\rg(C)=r$. Soit 
	\begin{equation*}
		\left\lbrace \lambda_1,\dots,\lambda_s\right\rbrace=\Sp_{\C}(A)\cap\Sp_{\C}(B),
	\end{equation*}
	avec $\lambda_1,\dots,\lambda_s$ deux à deux distincts, et 
	\begin{equation*}
		\begin{aligned}
			\chi_A &= (X-\lambda_1)^{m_1}\dots(X-\lambda_s)^{m_s}Q(X),\\
			\chi_B &= (X-\lambda_1)^{n_1}\dots(X-\lambda_s)^{n_s}R(X),
		\end{aligned}
	\end{equation*}
	avec pour tout $i\in\left\llbracket 1,s\right\rrbracket$, $Q(\lambda_i)\neq 0$ et $R(\lambda_i)\neq0$. Montrer que $r\leqslant\sum_{i=1}^{s}\min(m_i,n_i)$.
\end{exercise}
