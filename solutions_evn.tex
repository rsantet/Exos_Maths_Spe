...
\begin{proof}
	...
	On a 
	\begin{align*}
		\vert\varphi(x,y)-\varphi(x',y')
		&=\vert\Vert x-y\Vert-\Vert x'-y'\Vert\vert,\\
		&\leqslant\Vert (x-y)-(x'-y')\Vert,\\
		&\leqslant\Vert x-x'\Vert+\Vert y-y'\Vert,\\
		&\leqslant2\Vert(x,y)-(x',y')\Vert_{\infty}
	\end{align*}
	donc $\varphi$ est continue.

	$U\times V$ est compact, donc il existe $(x_{1},y_{1})\in(U\times V)$ telle que \begin{equation*}
		\varphi(x_{1},y_{1})=\min\limits_{(x,y)\in U\times V}\varphi(x,y).	
	\end{equation*}
	Comme $U$ et $V$ sont disjoints, $x_{1}\neq y_{1}$ et $\varphi(x_{1},y_{1}))d(U,V)>0$.
...
\begin{remark}
	Exemple pour l'exercice précédent : les fonctions polynômiales qui sont non constantes. Contre-exemple: l'exponentielle, cf $\exp([0,1])=\R_{-}$ non compact.
\end{remark}
...
\begin{proof}
	\phantom{}
	\begin{enumerate}
		\item $u$ est linéaire, et on a $\left\lvert u(f)\right\rvert\leqslant\left\lVert f\right\rVert_{\infty}$.
		\item Si $f_n$ est affine par morceaux valant 1 sur $\left[0,\frac{1}{2}-\frac{1}{n}\right]$ et -1 sur $\left[\frac{1}{2}+\frac{1}{n}\right]$, alors $\left\lVert f_n\right\rVert_{\infty}=1$ et $u(f_n)=1-\frac{1}{n}\xrightarrow[n\to+\infty]{}1$. Donc $\left\lVert u\right\rVert=1$. Si $f\in E$ avec $\left\lVert f\right\rVert_{\infty}=1$ est telle que $\left\lvert u(f)\right\rvert=1$, alors $\int_{0}^{1/2}f$ et $\int_{1/2}^{1}f$ sont de signes opposés. Quitte à échanger $f\mapsto -f$, on suppose $\int_{0}^{1/2}f\geqslant0$. Alors 
		\begin{equation*}
			\left\lvert\int_{0}^{1/2}f\right\rvert=\int_{0}^{1/2}\left\lvert f\right\rvert=\int_{0}^{1/2}f,
		\end{equation*}
		donc $\int_{0}^{1/2}\left\lvert f\right\rvert-f=0$. Donc $f$ garde un signe constant sur $\left[0,\frac{1}{2}\right]$ ($f\geqslant0$) et sur $\left[\frac{1}{2},1\right]$ ($f\leqslant0$). Enfin, $\int_{0}^{1/2}f=\frac{1}{2}=-\int_{1/2}^{1}f$, donc $\int_{0}^{1/2}1-f=0$ et $1-f\geqslant0$. Donc $f=1$ sur $\left[0,\frac{1}{2}\right]$ et de même $f=-1$ sur $\left[\frac{1}{2},1\right]$. C'est absurde car $f$ est continue.
	\end{enumerate}
\end{proof}
