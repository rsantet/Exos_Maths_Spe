...
\begin{solution}
	...
	\begin{enumerate}
		...
		\end{equation}
		la dernière équivalence venant du théorème de Gauss. Donc pour tout $i\in\left\llbracket 1,r\right\rrbracket$, $p_{i}^{\alpha_{i}}\mid k$, ce qui équivaut donc à $N\mid k$ et donc $\omega(x)=N$.
	\end{enumerate}
\end{solution}

\begin{solution}
	Sur un corps commutatif, un polynôme de degré $n$ admet au plus $n$ racines. Montrons qu'il existe $x_{1}\in\K^{*}$ tel que $\omega(x_{i})=\vert\K^{*}\vert$. Par définition de $N$, pour tout $x\in\K^{*}$, $\omega(x)\mid N$. D'où $x^{N}=1_{\K}$. Donc $x$ est racine de $X^{N}-1$. Ainsi, $\vert\K^{*}\vert\leqslant N$. Par ailleurs, $N\mid\vert\K^{*}\vert$ car pour tout $x\in\K^{*}$, $x^{\vert\K^{*}\vert}=1_{\K^{*}}$. Donc $\vert\K^{*}\vert=N$ et donc $\K^{*}=gr\left\{x_{1}\right\}$.
...
\begin{solution}
	\phantom{}
	\begin{enumerate}
		\item Si $G$ est abélien, on a $D(G)=\left\{e_{G}\right\}$.
		...
		Si $n\geqslant5$ et $\sigma$ impair, soit $(c_{1},c_{2})\in\left\llbracket 1,n\right\rrbracket\setminus\left\{a_{1},a_{2},a_{3}\right\}$. $\sigma'=\sigma\circ[c_{1},c_{2}]$ est pair et $\sigma'(a_{i})=b_{i}$. Donc les trois cycles sont conjugués dans $\mathcal{A}_{n}$ pour $n\geqslant5$. C'est cependant faux pour $n=3$ et $n=4$.
...
\begin{solution}
	...
	\begin{enumerate}
		...
		\item Soit $p$ un facteur premier de $F_{n}$. S'il existe $k\in\left\llbracket 0,n-1\right\rrbracket$ tel que $p\mid F_{k}$, alors d'après la première question on a $p\mid F_{n}-\prod_{k=0}^{n-1}F_{k}=2$. Donc $p=2$. Or $F_{n}$ est impair, donc non divisible par deux, ce qui est absurde. Donc $p$ ne divise aucun $F_{k}$ pour $k\in\left\llbracket 0,n-1\right\rrbracket$ et il existe donc une infinité de nombres premiers (car les $F_{n}$ sont tous différents deux à deux).
	\end{enumerate}
\end{solution}
...
\begin{solution}
	...
	\begin{enumerate}
		...
		\end{equation}
		Si $x_{n+1}\geqslant x_{n}$, alors $y_{n+1}\geqslant y_{n}$ d'où $x_{n+1}+\sqrt{2}y_{n+1}>x_{n}+\sqrt{2}y_{n}$ ce qui est absurde. Donc $x_{n+1}<x_{n}$ et on obtient une suite strictement décroissante d'entiers naturels ce qui est impossible. Donc $\beta>1$ et $E=\left\{(x_{0}+y_{0}\sqrt{2})^{n}\bigm| n\in\Z\right\}$ est monogène.
...
\begin{solution}
	...
	\begin{enumerate}
		...
		\end{align}
		Or $c(Q)c(R)=c(P)$ d'après le lemme de Gauss et nécessairement, $c(P)=1$. Donc $P=Q_{1}R_{1}$, et alors $Q_{1}=\pm 1$ et $R_{1}=\pm 1$, et $Q$ ou $R$ est constant, donc $P$ est irréductible sur $\Q[X]$.
...
\begin{solution}
	...
	\begin{align}
		...
		&\Longleftrightarrow\left(\sum_{i=1}^{n}\frac{1}{(X-a_{i})}\right)^{2}\leqslant n\times\sum_{i=1}^{n}\frac{1}{(X-a_{i})^{2}}
	\end{align}
...
\begin{solution}
	...
	\begin{enumerate}
		...
		Soit $x\in\K^{*}$ d'ordre $d$. On a $x\in gr\left\{x_{0}\right\}$ car $x^{d}=1$ (racine du polynôme de $X^{d}=1_{\K}$). Or, dans le groupe cyclique engendré par $x_{0}$, il y a $\varphi(d)$ éléments.
...
\begin{solution}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			On a $\frac{z_{1}}{z_{2}}\in\C$ donc d'après 3.(a), il existe $q\in\Z[\i]$ tel que $\left\lvert \frac{z_{1}}{z_{2}}-q\right\rvert\leqslant\frac{\sqrt{2}}{2}<1$. On pose alors $r=z_{1}-qz_{2}\in\Z[\i]$ par stabilité. Il vient donc $\vert r\vert<\vert z_{2}\vert$

			Il n'y a pas unicité: par exemple $z_{2}=1$ et $z_{1}=\frac{1+\i}{2}$.\\On peut prendre $q\in\left\{0,1,\i,1+\i\right\}$.
...
\begin{solution}
	...
	\begin{enumerate}
		...
		\end{equation}
		donc $R$ est un sous groupe de $(\mathbb{F}_{p}^{*},\times)$. Soit \function{\varphi}{\mathbb{F}_{p}^{*}}{\mathbb{F}_{p}^{*}}{\overline{y}}{\overline{y}^{2}}
